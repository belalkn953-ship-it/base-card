...
const FALLBACK_TRANSFER={4:{id:4,name:'SYRIATEL',params:['رقم الهاتف'],price:1.1372781},11:{id:11,name:'MTN',params:['رقم الهاتف'],price:1.138517265}};
...
function comparablePrice(p,q){const map=overrides();for(const k of [`${p.id}:${q}`,`${p.id}_${q}`,`${p.id}-${q}`]){const n=Number(map[k]);if(Number.isFinite(n)&&n>=0)return n+TRANSFER_MARGIN_SYP}const v=p?.sale_price_syp??p?.comparable_price_syp??p?.provider_price_syp??p?.amount_syp;if(v!==undefined&&v!==''&&Number.isFinite(Number(v)))return Number(v)+TRANSFER_MARGIN_SYP;const qn=Number(q);if(isTransfer(p)&&Number.isFinite(qn)&&qn>0)return Number(p.id)===11?Math.ceil(qn*1.15):Math.ceil(qn+10);return null}
...
async function submitKmOrder(p,qty,params,note,button){
 ...
 try{
  ...
  let state=r.data?.status||'processing'; const id=r.data?.order_id;
