...
function showChat(g){
 ...
 panel.innerHTML=`<h3>💬 ${kmEsc(g.name)}</h3><p class="muted">اختر الباقة وأدخل المعرّف المطلوب.</p><div id="chatPackages" class="packages-grid">${g.products.map(p=>packageCard(p)).join('')}</div><form id="chatPurchaseForm"><label class="full">معرّف المستخدم *<input id="chatTarget" required maxlength="160"></label><label class="full">الكمية *<input id="chatQty" type="number" min="1" step="any" required></label><button class="btn" type="submit">شراء وشحن الآن</button><p id="chatFormNote" class="muted" role="status"></p></form>`;
