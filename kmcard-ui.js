...
const kmEsc=v=>String(v??'').replace(/KM\s*Card|كيم\s*كارد/ig,'خدمة الشحن').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function ensureSections(){
 ...
 if(!km$('#chatapps'))back.insertAdjacentHTML('afterend','<section id="chatapps" class="section" data-main-section="chatapps"><div class="container"><div class="section-title"><div><label>خدمات الألعاب والتطبيقات</label><h2>تطبيقات الدردشة</h2></div><p>اختر التطبيق لعرض الباقات.</p></div><div id="chatappsGrid" class="games-grid"><div class="panel km-status">جارٍ تحميل تطبيقات الدردشة...</div></div></div></section>');
 if(!km$('#transfer'))back.insertAdjacentHTML('afterend','<section id="transfer" class="section" data-main-section="transfer"><div class="container"><div class="section-title"><div><label>خدمات الألعاب والتطبيقات</label><h2>تحويل الرصيد</h2></div><p>اختر سيريتل أو MTN لعرض باقاتهما المباشرة.</p></div><div id="transferGrid" class="games-grid"></div><div id="transferFormPanel" class="panel hidden" style="margin-top:16px"></div></div></section>');
...
async function submitKmOrder(p,qty,params,note,button,toastDuration=2000){
 ...
 try{
  ...
  const successMsg=`تم إنشاء الطلب ${r.data?.order_number||r.data?.order_id||id||'—'} — الحالة: ${state==='completed'?'مكتمل':state==='rejected'?'مرفوض وتم إرجاع الرصيد':'قيد المعالجة'}`;note.textContent=successMsg;if(typeof flashMessage==='function')flashMessage(successMsg,state==='rejected'?'error':'success',toastDuration);
  ...
  if(id&&state!=='completed'&&state!=='rejected') for(let i=0;i<8;i++){
    ...
    state=c.data?.status||state; const updateMsg=`الطلب ${r.data?.order_number||''} — الحالة: ${state==='completed'?'مكتمل':state==='rejected'?'مرفوض وتم إرجاع الرصيد':'قيد المعالجة'}`;note.textContent=updateMsg;if(typeof flashMessage==='function')flashMessage(updateMsg,state==='rejected'?'error':'success',toastDuration);
...
function showTransfer(provider){
 ...
 if(!p){panel.innerHTML='<p class="km-status error">منتج التحويل غير متاح حاليًا.</p>';panel.classList.remove('hidden');return}
 ...
 panel.innerHTML=`<h3>شراء رصيد ${kmEsc(provider)}</h3><form id="transferPurchaseForm" novalidate><label class="full">رقم الهاتف السوري *<input id="transferTarget" name="phone" inputmode="tel" required placeholder="09xxxxxxxx أو +9639xxxxxxxx"><small class="muted">مطلوب لإتمام الشراء.</small></label><p class="muted">اختر الكمية التي تريد شراءها من القائمة التالية.</p><div id="transferPackages" class="packages-grid">${cards}</div><button class="btn" type="submit">شراء وشحن الآن</button><p class="muted duplicate-order-warning" style="margin-top:12px;text-align:center;color:#f59e0b;font-weight:700">لا تضغط على زر الشراء مرتين حتى لا يتم خصم رصيد إضافي من محفظتك.</p><p id="transferFormNote" class="muted" role="status"></p></form>`;
...
function showChat(g){
 ...
 panel.innerHTML=`<h3>💬 ${kmEsc(g.name)}</h3><p class="muted">اختر الباقة وأدخل المعرّف المطلوب.</p><div id="chatPackages" class="packages-grid">${g.products.map(p=>packageCard(p)).join('')}</div><form id="chatPurchaseForm"><label class="full">معرّف المستخدم *<input id="chatTarget" required maxlength="160"></label><label class="full">الكمية *<input id="chatQty" type="number" min="1" step="any" required></label><button class="btn" type="submit">شراء وشحن الآن</button><p class="muted duplicate-order-warning">لا تضغط على زر الشراء مرتين حتى لا يتم خصم رصيد إضافي من محفظتك.</p><p id="chatFormNote" class="muted" role="status"></p></form>`;
 const form=km$('#chatPurchaseForm');form.addEventListener('submit',e=>{e.preventDefault();const sel=form.querySelector('.km-package.selected'),note=km$('#chatFormNote');if(!sel){note.textContent='اختر الباقة أولًا.';return}const p=g.products.find(x=>Number(x.id)===Number(sel.dataset.kmId));const key=String(p?.params?.[0]||'User ID');submitKmOrder(p,Number(km$('#chatQty').value),{[key]:km$('#chatTarget').value.trim()},note,form.querySelector('button[type=submit]'),5000)});
...
async function load(){const cg=km$('#chatappsGrid');if(cg)cg.innerHTML='<div class="panel km-status">جارٍ تحميل تطبيقات الدردشة...</div>';try{if(typeof sb==='undefined'||!sb.functions?.invoke)throw Error('تعذر تهيئة خدمة المزود');const r=await sb.functions.invoke(PROXY,{body:{action:'products'}});if(r.error)throw Error(r.error.message||'تعذر الاتصال بمزود الخدمة');const list=parse(r.data).filter(p=>p&&p.available!==false&&p.id!=null&&!isServer2Product(p));if(!list.length)throw Error('لم تصل المنتجات المتاحة حاليًا');state.products=list;state.groups.chat=group(list.filter(isChat));draw();window.__kmProducts=list}catch(e){draw();const msg=kmEsc(e.message||'حدث خطأ من مزود الخدمة.');document.querySelectorAll('#chatappsGrid').forEach(x=>x.innerHTML=`<div class="panel km-status error"><strong>تعذر تحميل منتجات الدردشة</strong><br>${msg}</div>`);}}
ensureSections();
